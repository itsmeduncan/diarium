...
**Prior stage:** `docs/superpowers/plans/2026-08-21-streaming-edition-stage-a.md` (the v5 format library it builds on)
...
- Little-endian; reuse Stage A's writer/reader. Commit messages: imperative + the `Co-Authored-By`/`Claude-Session` trailers.
...
Switch the device to the streaming write and lazy read, and prove the memory on hardware. **This task needs the physical board and is run by the human operator, not a subagent.**
...
**Note for the executor:** Tasks 1–4 are host-testable and can be implemented by a subagent. Task 5 must be run by the human operator on the physical device — a subagent cannot flash or measure the board, and this is exactly where "works in the sim, breaks on hardware" has bitten this project before.
