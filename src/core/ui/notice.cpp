...
void render_sleep_page(const FontPack& fonts, const std::string& title,
  ...
  fb->fill_rect(0, band_top - 22, page_width(), 7, kInk);
  fb->fill_rect(0, band_top - 10, page_width(), 2, kInk);
  fb->fill_rect(0, band_top + band_height + 8, page_width(), 2, kInk);
  fb->fill_rect(0, band_top + band_height + 15, page_width(), 7, kInk);
  ...
  const int bar_y = page_height() - 84;
  ...
  if (meta.valid()) {
    ...
    const int x = (page_width() * kSubpixel - measured) / 2;
    fb->draw_text_tracked(meta, hint, x, page_height() - 28, 110, tracking);
  }
}
