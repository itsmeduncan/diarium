...
// wakes worth measuring are the unattended ones, and readBattery() samples the
// battery terminal through a divider (EPDDriver::readBattery) — so a
// host that plugs in tomorrow to read a voltage has also put the cell back on
// charge, and gets the charger's 4.2 V whatever the night actually cost.
void log_battery(device::DeviceHal& d, int mv) {
  ...
  // appended a few times a day, but "a few" is an assumption, and an
  // unbounded file sharing a card with the edition is not acceptable.
...
void setup() {
  ...
  // On every boot, not only the ones with a host attached: a day's draw is the
  // difference between two of these, and the wakes in between are exactly the
  // ones nobody is watching. Before the card is mounted, because a flat
  // battery is a plausible reason for the card to fail and this should be on
  // the line either way. Reader's low_battery_mv is still a guess; this is the
  // only thing that can turn it into a measurement.
  ...
  // return, so a 05:30 wake logged any later would not be logged at all.
